		...
		for(var ke in aTbInfo){
			...
			backMkr.addEventListener("click", function(e,d,w) { 
				...
				$('#radius option').each(function(){
					if(_this.code.radius == $(this).attr('data')) $('#radius').val($(this).val());
				});
...
function fnOK() {
	...
	if(localStorage.getItem("aTbInfo")){
		...
		//判断更新原来标注,返回除了相同经纬度的剩余元素
		aTbInfo = _.reject(aTbInfo, function(v){
			return v.jwd[0] == tanbaData.jwd[0] && v.jwd[1] == tanbaData.jwd[1];
		});
