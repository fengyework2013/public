...
function createTabel(ele, data) {
    ...
    $.each(d, function (item, ele) {
        ...
        $('<td></td>').text(ele['iAddTime']).appendTo($tr);
        ...
        $(".lookId").on("click", function () {
            ...
            $(pare).find(".tab-li5").addClass("active").siblings().removeClass("active");
            $.cookie('lookAggregationId', id);
            $.cookie("taskName", taskName);
            $.cookie("dateStart",dateStart);
            $.cookie("dataEnd",dateEnd);
            window.location.href = "aggregationLog.html";
        });
    });
...
//编辑
function set(id) {
    $.cookie('ifsetaggregation', 1);
    var index = layer.open({
        type: 2,
        title: "编辑集结预警任务",
        area: ['40%', '50%'],
        content: "addAggregation.html"
    });
    layer.full(index);
    $.cookie("aggregationId", id);
}
