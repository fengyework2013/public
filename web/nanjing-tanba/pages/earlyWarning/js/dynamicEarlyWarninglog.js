$(document).ready(function () {
    //任务名列表创建
    fnAjax.method_4(url_join3("taskaction/list/4"), '', "get", function (data) {
        if (data.code == 0) {
    ...
    $('#search').click(function () {
        ...
        active_layPage.method_post(
            url_join3("taskaction/showlog/4/" + taskName),
            "10",
            ".table-data",
            function (d) {
    ...
    function dynamic_page(){
        var lookdynamicId = $.cookie('lookdynamicId');
        var taskName = $.cookie('taskName');
        if (lookdynamicId) { // 判断是否点击查看进入页面
            active_layPage.method_get(
                url_join3("taskaction/showlog/4/" + lookdynamicId),
                "10",
                ".table-data",
                function (d) {
                    createTabel(".table-data", d.data.data);
