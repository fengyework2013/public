$(document).ready(function () {
    //任务名列表创建
    fnAjax.method_4(url_join3("taskaction/list/5"), '', "get", function (data) {
        if (data.code == 0) {
    ...
    $('#search').click(function () {
        ...
        active_layPage.method_post(
            url_join3("taskaction/showlog/5/" + taskName),
            "10",
            ".table-data",
            function (d) {
                setTimeout(function () {
    ...
    function dynamic_page() {
        var lookAggregationId = $.cookie('lookAggregationId');
        var taskName = $.cookie('taskName');
        if (lookAggregationId) { // 判断是否点击查看进入页面
            active_layPage.method_get(
                url_join3("taskaction/showlog/5/" + lookAggregationId),
                "10",
                ".table-data",
                function (d) {
                    createTabel(".table-data", d.data.data);
