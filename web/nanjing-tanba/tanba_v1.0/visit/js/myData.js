	...
	if("WebSocket" in window) {
		...
		function TerminalList(msg) {
			...
			for(var i = 0; i < deviceArrLegth; i++) {
				if(_this.deviceArr[i] == msg.deviceID) {
					addBool = false;
					break;
				}
			};
			if(addBool) {
				_this.deviceArr.push(msg.deviceID);
				var liStr = '<li>' +
					'<div class="icon"><i class="glyphicon glyphicon-hdd"></i></div>' +
					'<div class="deviceId">' +
					'<span class="title">设备ID：</span>' +
					'<span class="content">' + msg.deviceID + '</span>' +
					'</div>' +
					'</li>';
		...
		function UploadInfo(msg) {
			...
			if(bool) {
				if(data.urgentStatus == "紧急") {
					var urgentStatusStr = "<td class='error'>" + data.urgentStatus + "</td>";
				} else {
					var urgentStatusStr = "<td class='success'>" + data.urgentStatus + "</td>";
				}
				var tbodyStr = "<tr>" +
					"<td>" + msg.time + "</td>" +
					"<td>" + msg.deviceID + "</td>" +
...
deviceWebSocketObj.prototype.WIFIwebSocket = function() {
	if("WebSocket" in window) {
		...
		function TerminalList(msg) {
			...
			for(var i = 0; i < deviceArrLegth; i++) {
				if(_this.WIFIdeviceArr[i] == msg.deviceID) {
					addBool = false;
					break;
				}
			};
			if(addBool) {
				_this.WIFIdeviceArr.push(msg.deviceID);
				var liStr = '<li>' +
					'<div class="icon"><i class="glyphicon glyphicon-hdd"></i></div>' +
					'<div class="deviceId">' +
					'<span class="title">设备ID：</span>' +
...
deviceWebSocketObj.prototype.APwebSocket = function() {
	if("WebSocket" in window) {
		...
		function TerminalList(msg) {
			...
			for(var i = 0; i < deviceArrLegth; i++) {
				if(_this.APdeviceArr[i] == msg.deviceID) {
					addBool = false;
					break;
				}
			};
			if(addBool) {
				_this.APdeviceArr.push(msg.deviceID);
				var liStr = '<li>' +
					'<div class="icon"><i class="glyphicon glyphicon-hdd"></i></div>' +
					'<div class="deviceId">' +
					'<span class="title">设备ID：</span>' +
...
deviceWebSocketObj.prototype.queryCriteria = function(msg, query) {
	if(query == undefined || query == "") {
		...
		for(var keys in data) {
			if(keys == "deviceID") {
				if(data[keys].toString().indexOf(query) != -1) {
					msgBool = true;
					break;
				}
			} else {
