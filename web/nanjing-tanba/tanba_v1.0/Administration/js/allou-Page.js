(function(win, doc, $) {
	...
	$.extend(allotManagement.prototype, {
		...
		selPermissions: function(did) {
			...
			self.aJaxRequestEvent(url_join("Role/getRoleNode"), data, function(obj) {
				...
				if(obj.result == "success") {
					...
					$.each(obj.data, function(index, ele) {
						...
						$.each(ele.child, function(i, e) {
							...
							var inputStr = '';
							if(e.access == 1) {
								inputStr = '<input type="checkbox" parentid="' + e.parentid + '" data-id="' + e.id + '" class="childNodeCheckBox" checked/>';
							} else {
								inputStr = '<input type="checkbox" parentid="' + e.parentid + '" data-id="' + e.id + '" class="childNodeCheckBox"/>';
							}
